...
- One-time credit packs: map Clerk Price IDs to credit amounts in `src/lib/clerk/credit-packs.ts`. The Clerk webhook adds credits on `invoice.payment_succeeded` via `addUserCredits`.
