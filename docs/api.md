...
- Configure Clerk Price → credits mapping in `src/lib/clerk/credit-packs.ts`:
  ```ts
  export const CREDIT_PACK_PRICE_TO_CREDITS = {
    'cplan_small_pack': 100,
    'cplan_medium_pack': 500,
  }
  ```
...
Receives user CRUD events from trusted back-office systems. The endpoint expects an HMAC SHA-256 signature generated with a configured secret.

- **Headers**
  - `x-webhook-signature`: hex digest of `HMAC_SHA256(secret, "${timestamp}.${rawBody}")`
  - `x-webhook-timestamp`: Unix timestamp (seconds or milliseconds). Requests older than 5 minutes are rejected.
- **Payload**
  ```json
  {
    "id": "evt_123",
  ...
  ```
  - You can batch events by sending `{ "events": [ ... ] }`.
  - Supported types: `user.created`, `user.updated`, `user.deleted`.
  - Credits:
    - `initialCredits`: absolute amount applied on create/update (legacy name, still supported).
    - `creditsRemaining`: preferred absolute setter for the user's balance.
    - `creditDelta`: relative adjustment (positive = add credits, negative = deduct). Result never goes below zero.
- **Behavior**
  - Create: upserts the user, initializes `creditBalance` (defaults to 0 credits if omitted).
  - Update: syncs email/name/isActive; if the user does not exist yet, the handler creates it on the fly. Credits can be set directly via `creditsRemaining`/`initialCredits` or adjusted with `creditDelta`.
  - Delete: removes dependent records (usage history, credit balance, storage objects) before deleting the user row.

Security:
- Keep the webhook secret private; rotate it if you suspect leakage.
- The timestamp guard mitigates replay attacks—send events immediately and retry quickly if needed.
...
- `page` (number, optional): Page number (default: 1)
- `limit` (number, optional): Items per page (default: 20, max: 100)
- `search` (string, optional): Search by name, contentType, or url
...
One‑time Credit Packs:
- Configure Price→credits mapping in `src/lib/clerk/credit-packs.ts`. Keys should match the price IDs that appear on Clerk invoice line items (Clerk Price IDs):
  ```ts
  export const CREDIT_PACK_PRICE_TO_CREDITS = {
    'cplan_small_pack': 100,
    'cplan_medium_pack': 500,
