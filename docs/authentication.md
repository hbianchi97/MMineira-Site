...
### Alternative User Sync Webhook

When Clerk is not the source of truth, you can POST lifecycle events to `/api/webhooks/users`. The handler verifies signatures with a configured secret, accepts batched `{ events: [] }` payloads, and mirrors `user.created|user.updated|user.deleted` events into the local `User`/`CreditBalance` tables. It also cascades deletions (usage history, storage objects, balances) before removing the user.

Use `creditsRemaining` for absolute balances or `creditDelta` for relative adjustments (balances never drop below zero). This endpoint is ideal for syncing from an internal admin, legacy IdP, or background worker while retaining Clerk for auth.
