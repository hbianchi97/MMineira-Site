...
- Payload: either a single event or `{ "events": [...] }`, where every event has `type` (`user.created|user.updated|user.deleted`) and `data.id` (mapped to `User.clerkId`). Optional fields (`email`, `firstName`, `lastName`, `name`, `isActive`, `initialCredits`) update the corresponding columns.
- Credit fields:
...
- One-time Credit Packs: Users can purchase additional credits via billed price IDs mapped to credit amounts (Clerk Price IDs).
